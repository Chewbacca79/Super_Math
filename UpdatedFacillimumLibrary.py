...
class Facillimum_Library():
    ...
    def draw_image(
        ...
        """This function will draw a pygame Surface object at the inputed x and y coordinates onto the window. MODIFIED FOR LASER CHESS"""
        if type(image) == pygame.Surface:
            if type(coordinates) == tuple:
                ...
                size = image.get_size()
                rect = image.get_rect()
                rect.x = coordinates[0] - (size[0] / 2)
                rect.y = coordinates[1] - (size[1] / 2)
    ...
    def draw_words(self,
        ...
        font = pygame.font.SysFont("PublicPixel-z84yD", size)
        try:
            image = font.render(words, True, color)
            if shadow:
                shadow = font.render(words, True, (0,0,0))
                self.draw_image(shadow, (coordinates[0]+3, coordinates[1]+3), 3)
        except ValueError:
        ...
        self.draw_image(image, coordinates, 3)
